...
//            debugRing.GetComponent<MeshRenderer>().castShadows = false;
            ...
            defaultAngleX = config.GetFloat("DefaultAngleX");
            defaultAngleY = config.GetFloat("AngleY");
        }
        ...
        public override void OnActivate(Modes.Type previousMode)
        {
            ...
            if (preserveTransitionRotation && previousMode != Modes.Type.None)
            {
                ...
                defaultAngleX = rotX*-Mathf.Rad2Deg;
            }
        ...
        private void UpdateYAngle()
        {
            ...
            var yAngle = config.GetFloat("AngleZoomMin") * (1.0f - zoomRatio) + defaultAngleY * zoomRatio;
        ...
        private void UpdateFollow()
        {
            ...
            if (followParam > 0.0f)
            {
                ...
                var charForwardXZ = Utils.Math.VectorXZ(Target.forward);
                var camRightXZ = Utils.Math.VectorXZ(UnityCamera.transform.right);

                var deltaAngle = Vector3.Angle(camForwardXZ, charForwardXZ);

                var sined = deltaAngle*0.1f * Mathf.Sign(Vector3.Dot(camRightXZ, charForwardXZ));

                var rotDelta = sined * Time.deltaTime * followParam * targetVelocity * 0.2f;
        ...
        private void UpdateXAngle(bool force)
        {
            ...
            {
                rotX = defaultAngleX * -Mathf.Deg2Rad;
//                Utils.Debug.Log("UPdateXAngle: {0}", rotX);
        ...
        public override void PostUpdate()
        {
            ...
            if (InputManager)
            {
                ...
                //
                // update auto follow mode (aka 3rd peson camera style)
                //
                UpdateFollow();
