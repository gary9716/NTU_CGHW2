...
namespace RG_GameCamera.Modes
{
    ...
    public class OrbitCameraMode : CameraMode
    {
        ...
        public override void OnActivate(Modes.Type previousMode)
        {
            ...
            if (config.GetBool("UseInitialSettings"))
            {
                var pos = config.GetVector3("InitialPosition");
                var zoom = config.GetFloat("InitialZoom");

                UnityCamera.transform.position = pos;
                UnityCamera.transform.LookAt(cameraTarget);
                ZoomCamera(zoom);
            }
        }
...
//            UnityCamera.transform.LookAt(cameraTarget);
        ...
        private void LimitCamera()
        {
            if (config.GetBool("DragLimits"))
            {
                ...
                cpos.x = Mathf.Clamp(cpos.x, limitX.x, limitX.y);
                cpos.y = Mathf.Clamp(cpos.y, limitY.x, limitY.y);
                cpos.z = Mathf.Clamp(cpos.z, limitZ.x, limitZ.y);

                cameraTarget = cpos + UnityCamera.transform.forward*targetDistance;
