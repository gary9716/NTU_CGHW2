...
namespace RG_GameCamera.Modes
{
    ...
    public class ThirdPersonCameraMode : CameraMode
    {
        ...
        public float PosFilter = 1.0f;
        public float RotFilter = 1.0f;

        public enum SpringAlgorithm
        {
            SmoothDamp,
            Lerp,
            MovingAverage
        ...
        public SpringAlgorithm SpringType;
        ...
        private InputFilter rotationFilter;
...
//            debugRing.GetComponent<MeshRenderer>().castShadows = false;
            ...
            targetFilter = new PositionFilter(10, PosFilter);
            targetFilter.Reset(Target.position);

            rotationFilter = new InputFilter(10, RotFilter);
            rotationFilter.Reset(Vector2.zero);

            Utils.DebugDraw.Enabled = true;

            resetTimeout = 0.0f;

            //                var yRot = defaultAngleY;
            //                rotY = -yRot * Mathf.Deg2Rad;
            //                rotX = Mathf.Atan2(Target.forward.x, Target.forward.z);

            defaultAngleY = config.GetFloat("DefaultYRotation") * -Mathf.Deg2Rad;
            defaultAngleX = Mathf.Atan2(Target.forward.x, Target.forward.z);
        }

        public override void OnActivate(Modes.Type previousMode)
        {
            base.OnActivate(previousMode);
            ...
            if (preserveTransitionRotation && previousMode != Modes.Type.None)
            {
        ...
        {
            rotationFilter.SetWeight(RotFilter);
            rotationFilter.AddSample(mousePosition);

            var inputPos = rotationFilter.GetValue();

            rotationInput = inputPos.sqrMagnitude > Mathf.Epsilon;
            ...
            rotY += config.GetFloat("RotationSpeedY") * inputPos.y * 0.01f;
            rotX += config.GetFloat("RotationSpeedX") * inputPos.x * 0.01f;
        ...
        {
            var p = targetPos + GetOffsetPos();
            //cameraTarget = Utils.Interpolation.LerpS3(newTarget, GetTargetHeadPos(), currCollisionTargetDist);
            cameraTarget = Vector3.Lerp(p, GetTargetHeadPos(), 1.0f - currCollisionTargetDist);
        ...
        void UpdateTarget()
        {
            ...
            else if (spring > 0.0f)
            {
                switch (SpringType)
                {
                    case SpringAlgorithm.SmoothDamp:
                        targetPos = Vector3.SmoothDamp(targetPos, newTarget, ref springVelocity, spring);
                        break;

                    case SpringAlgorithm.Lerp:
                        targetPos = Vector3.Lerp(targetPos, newTarget, Time.deltaTime * 10 * spring);
                        break;

                    case SpringAlgorithm.MovingAverage:
                    {
                        targetPos += (newTarget - targetPos)*spring;
                    }
                    break;
                }
            }
...
//            Utils.Debug.Log("targetDistance: {0} collisionDistance: {1}", targetDistance, collisionDistance);
        ...
        {
            targetFilter.SetWeight(PosFilter);

            // filter target position for smoother result
            if (PosFilter > 0.0f)
            {
                targetFilter.AddSample(Target.position);
                newTarget = targetFilter.GetValue();
            }
