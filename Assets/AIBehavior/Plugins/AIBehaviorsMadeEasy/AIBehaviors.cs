...
namespace AIBehavior
{
	...
	public class AIBehaviors : AIComponent
	{
		...
		protected Transform lastKnownTarget;
		...
		public void MoveAgent(Vector3 targetPoint, float targetSpeed, float rotationSpeed, Transform rotationTargetOverride = null)
		{
			...
			if ( isNavAgent && navMeshAgent.enabled )
			{
				...
				{
					velocity = (rotationTargetOverride.position - thisTFM.position).normalized;
				}
				...
				transform.Rotate(Vector3.Cross(thisTFM.forward, velocity) * Time.deltaTime * rotationSpeed * rotationMultiplier);
			}
		...
		protected virtual void RotateAgent()
		{
			bool isNavAgent = navMeshAgent != null;
		...
		public Vector3 GetSightPosition (Transform sightTransformOverride = null)
		{
			...
			if (sightTransformOverride == null)
			{
				return transform.position + eyePosition;
			}
		...
		public void PlayAudio()
		{
			...
			audioSource.loop = currentState.loopAudio;
			audioSource.volume = currentState.audioVolume;
			audioSource.pitch = currentState.audioPitch + ((Random.value * currentState.audioPitchRandomness) - (currentState.audioPitchRandomness / 2.0f)) * 2.0f;
			audioSource.clip = currentState.audioClip;
